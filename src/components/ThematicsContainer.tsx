...
export default function ThematicsContainer() {
  ...
  const handleMenuClick = (event: React.MouseEvent, item: typeof menuData[0]) => {
    if (item.type === 'modal' && item.contentKey) {
      event.preventDefault();
      const content = thematicsData[item.contentKey as keyof typeof thematicsData];
  ...
  const getRandomVoice = () => {
    ...
    let filteredVoices = availableVoices.filter(function(voice: any) {
